...
Write-Host '15 - Change UPN Domain O365 - All users'
...
function displayEmailAlias {
    ...
    Get-Recipient | Select DisplayName, RecipientType, EmailAddresses | Export-Csv EmailAddresses.csv | Export-Csv C:\Users\%Username%\Desktop\email-aliases.csv
...
function changeUPNBulk {
    
    Connect-MsolService

    $newDomain = Read-Host 'Please enter the new domain'

    Get-MsolUser -all | Where { -Not $_.UserPrincipalName.ToLower().StartsWith("admin@") } | ForEach { Set-MsolUserPrincipalName -ObjectId $_.ObjectId -NewUserPrincipalName ($_.UserPrincipalName.Split("@")[0] + "@" + $newDomain) }
